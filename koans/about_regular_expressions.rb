...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_a_character_class_can_be_negated
    assert_equal "the number is\s", "the number is 42"[/[^0-9]+/]
  end

  def test_shortcut_character_classes_are_negated_with_capitals
    assert_equal "the number is\s", "the number is 42"[/\D+/]
    assert_equal 'space:', "space: \t\n"[/\S+/]
    # ... a programmer would most likely do
    assert_equal "\s=\s", "variable_1 = 42"[/[^a-zA-Z0-9_]+/]
    assert_equal "\s=\s", "variable_1 = 42"[/\W+/]
  end
  ...
  def test_caret_anchors_to_the_start_of_lines
    # strange working as multilines search by default, why?
  ...
  def test_sub_is_like_find_and_replace
    # but sorry it is really wtf moment
